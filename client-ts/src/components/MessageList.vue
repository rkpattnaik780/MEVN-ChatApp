    ...
    <v-card class="message-box">
      <template v-for="(message, index) in messages">
        <v-layout
          ...
          :key="index"
          class="message"
          v-if="message.username == currentUser.username"
        >
          ...
          <v-flex xs1 class="text-xs-center">
            <v-tooltip dark left>
              ...
              <div>
                <span>{{ moment(message.time).format("hh:mm:ss A") }}</span
                ><br />
...
import Vue from "vue";
import { UserMessageDTO } from "@/interfaces/message.model.dto";
...
export default Vue.extend({
  ...
  data() {
    return {
      messages: [] as UserMessageDTO[]
    };
  },
  mounted() {
    this.fetchMessages();
    this.socket.on(Events.MessagesFetched, (data: UserMessageDTO[]) => {
      this.messages = data;
    });
  },
  methods: {
    fetchMessages() {
      this.socket.emit(Events.FetchMessages);
    }
  }
});
</script>
